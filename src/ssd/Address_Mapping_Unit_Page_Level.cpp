...
namespace SSD_Components
{
	...
	AddressMappingDomain::AddressMappingDomain(unsigned int cmt_capacity, 
		...
		unsigned int sectors_per_mu) :
			...
			Die_no(die_no), 
			Plane_no(plane_no),
			mru_channel_id (channel_no - 1),
			mru_chip_id (chip_no - 1),
			mru_die_id (die_no - 1),
			mru_plane_id (plane_no - 1)
	{
		Total_physical_pages_no = total_physical_sectors_no / sectors_per_mu;
		max_logical_sector_address = total_logical_sectors_no;
		Total_logical_pages_no = (max_logical_sector_address / sectors_per_mu) + (max_logical_sector_address % sectors_per_mu == 0? 0 : 1);
	...
	inline PPA_type AddressMappingDomain::Get_ppa(const bool ideal_mapping, const stream_id_type stream_id, const LPA_type lpa)
	{
		if (ideal_mapping){
			...
			CMT->Retrieve_ppa(stream_id, lpa);
		}
			...
			fold_large_addresses)
	{
		...
		for (unsigned int domainID = 0; domainID < no_of_input_streams; domainID++) {
			...
			*/
			CMT_entry_size = (unsigned int)std::ceil(((2 * std::log2(total_physical_pages_no)) + sectors_per_mu) / 8);
			//In GTD we do not need to store lpa
			GTD_entry_size = (unsigned int)std::ceil((std::log2(total_physical_pages_no) + sectors_per_mu) / 8);
			no_of_translation_entries_per_page = (sectors_per_mu * SECTOR_SIZE_IN_BYTE) / GTD_entry_size;
			...
			if ( stream_channel_ids[domainID].size()!= channel_count ) PRINT_ERROR("Invalid channel ID specified for I/O flow " << domainID);
			...
			if ( stream_chip_ids[domainID].size()!= chip_no_per_channel ) PRINT_ERROR("Invalid chip ID specified for I/O flow " << domainID);
			...
			if ( stream_die_ids[domainID].size()!= die_no_per_chip ) PRINT_ERROR("Invalid die ID specified for I/O flow " << domainID);
			...
			if ( stream_plane_ids[domainID].size()!= plane_no_per_die ) PRINT_ERROR("Invalid chip ID specified for I/O flow " << domainID);
			...
			domains[domainID] = new AddressMappingDomain(per_stream_cmt_capacity, CMT_entry_size, no_of_translation_entries_per_page,
				...
				channel_ids, 
				(unsigned int)(stream_channel_ids[domainID].size()), 
				chip_ids, 
				(unsigned int)(stream_chip_ids[domainID].size()), 
				die_ids, 
				(unsigned int)(stream_die_ids[domainID].size()), 
				plane_ids, (unsigned int)(stream_plane_ids[domainID].size()),
...
/*
		...
		NVM_Transaction_Flash_WR* dummy_tr = new NVM_Transaction_Flash_WR(Transaction_Source_Type::MAPPING, 0, 0, NO_LPA, 0, NULL, 0, NULL, 0, 0);
		...
		for (unsigned int stream_id = 0; stream_id < no_of_input_streams; stream_id++) {
			...
			for (MVPN_type translation_page_id = 0; translation_page_id < domains[stream_id]->Total_translation_pages_no; translation_page_id++) {
				dummy_tr->set_lpa((LPA_type)translation_page_id);
				allocate_plane_for_translation_write(dummy_tr);
				allocate_page_in_plane_for_translation_write(dummy_tr, (MVPN_type)dummy_tr->get_lpa(), false);
				flash_controller->Change_flash_page_status_for_preconditioning(dummy_tr->Address, dummy_tr->get_lpa() );
			}
		} 
*/
	}
	...
	void Address_Mapping_Unit_Page_Level::Translate_lpa_to_ppa_and_dispatch(const std::list<NVM_Transaction*>& transactionList)
	{
		for (std::list<NVM_Transaction*>::const_iterator it = transactionList.begin();
			...
			if (is_lpa_locked_for_gc((*it)->Stream_id, ((NVM_Transaction_Flash*)(*it))->get_lpa())) {
		...
		if (transactionList.size() > 0) 
		{
				...
				it != transactionList.end(); it++) 
			{
				if (((NVM_Transaction_Flash*)(*it))->Physical_address_determined) 
				{
					...
					{
						NVM_Transaction_Flash* transaction = (NVM_Transaction_Flash*)(*it);
						//FGM - Transactions are ready to send to TSU
						for (unsigned int mu_idx=0; mu_idx<transaction->num_lpas(); mu_idx++)
						{
							if (((NVM_Transaction_Flash_WR*)(transaction))->get_related_read(mu_idx) != NULL) 
							{
								ftl->TSU->Submit_transaction(((NVM_Transaction_Flash_WR*)(*it))->get_related_read(mu_idx) );
							}
						}
					}
				}
	...
	bool Address_Mapping_Unit_Page_Level::query_cmt(NVM_Transaction_Flash* transaction)
	{
		...
		if (domains[stream_id]->Mapping_entry_accessible(ideal_mapping_table, stream_id, transaction->get_lpa()))//Either limited or unlimited CMT
		{
			...
			if (request_mapping_entry(stream_id, transaction->get_lpa())) {
				Stats::CMT_miss++;
				...
				if (transaction->Type == Transaction_Type::READ) {
					...
					Stats::readTR_CMT_miss++;
					Stats::readTR_CMT_miss_per_stream[stream_id]++;
					domains[stream_id]->Waiting_unmapped_read_transactions.insert(std::pair<LPA_type, NVM_Transaction_Flash*>(transaction->get_lpa(), transaction));
				} else {//This is a write transaction
					Stats::total_writeTR_CMT_queries++;
					Stats::total_writeTR_CMT_queries_per_stream[stream_id]++;
					Stats::writeTR_CMT_miss++;
					Stats::writeTR_CMT_miss_per_stream[stream_id]++;
					domains[stream_id]->Waiting_unmapped_program_transactions.insert(std::pair<LPA_type, NVM_Transaction_Flash*>(transaction->get_lpa(), transaction));
				}
			}
	...
	*/
	...
	{
		//FGM - Allocation of a page for a user/gc write transaction
		if ( transaction->Type == Transaction_Type::WRITE && transaction->num_lpas() != mu_per_page )
			PRINT_ERROR("Address Mapping Unit should process merged write tarnsactions only.");
		for (unsigned int mu_idx=0; mu_idx<transaction->num_lpas(); mu_idx++)
		{
			PPA_type ppa = domains[streamID]->Get_ppa(ideal_mapping_table, streamID, transaction->get_lpa(mu_idx));
			if (transaction->Type == Transaction_Type::READ) {
				if (ppa == NO_PPA) {
					ppa = online_create_entry_for_reads(transaction->get_lpa(mu_idx), streamID, transaction->Address, ((NVM_Transaction_Flash_RD*)transaction)->read_sectors_bitmap);
				}
				transaction->set_ppa(ppa, mu_idx);
				Convert_ppa_to_address(transaction->get_ppa(mu_idx), transaction->Address);
	...
	void Address_Mapping_Unit_Page_Level::allocate_page_in_plane_for_user_write(NVM_Transaction_Flash_WR* transaction, bool is_for_gc)
	{
		...
		//FGM - Iterate through the LPAs of the merged transaction 
		for (unsigned int mu_idx=0; mu_idx<transaction->num_lpas(); mu_idx++)
		{
			PPA_type old_ppa = domain->Get_ppa(ideal_mapping_table, transaction->Stream_id, transaction->get_lpa(mu_idx) );	
			bool flag = false;
			...
			if (old_ppa == NO_PPA)  /*this is the first access to the logical page*/
			{
				if (is_for_gc) 
				{
					PRINT_ERROR("Unexpected mapping table status in allocate_page_in_plane_for_user_write function for a GC/WL write => old ppa == NO PPA!")
				}
			} else {
				if (is_for_gc) 
				{
					...
					page_status_type prev_page_status = domain->Get_page_status(ideal_mapping_table, transaction->Stream_id, transaction->get_lpa(mu_idx) );
					page_status_type status_intersection =  transaction->get_sector_bitmap(0, sectors_per_mu) & prev_page_status;
					...
					{
						//FGM - Related Read in the making in order to service the Read-Modify-Write operations
						page_status_type read_pages_bitmap = status_intersection ^ prev_page_status;
						NVM_Transaction_Flash_RD *update_read_tr;

						if (!flag)
						{
							//FGM - New RD transaction
							update_read_tr = new NVM_Transaction_Flash_RD(
								transaction->Source, 
								transaction->Stream_id,
								count_sector_no_from_status_bitmap(read_pages_bitmap) * SECTOR_SIZE_IN_BYTE, 
								transaction->get_lpa(mu_idx),
								old_ppa, 
								transaction->UserIORequest,
								transaction->Content, 
								transaction, 
								read_pages_bitmap, 
								domain->GlobalMappingTable[transaction->get_lpa(mu_idx)].TimeStamp);
							...
							transaction->set_related_read(update_read_tr);
						}
						if (flag)
						{
							update_read_tr->update_transaction(
								count_sector_no_from_status_bitmap(read_pages_bitmap)* SECTOR_SIZE_IN_BYTE,
								transaction->get_lpa(mu_idx),
								old_ppa,
								sectors_per_mu,
		...
		* may decide to move a page that is just invalidated.*/
		...
		//FGM - Merged transaction ready to be assigned with a PPA
		for (unsigned int mu_idx=0; mu_idx<transaction->num_lpas(); mu_idx++)
		{
			transaction->Address.MUID = mu_idx;
			PPA_type ppa = Convert_address_to_ppa(transaction->Address);
			transaction->set_ppa(ppa,mu_idx); 
			domain->Update_mapping_info(ideal_mapping_table, 
				transaction->Stream_id, 
				transaction->get_lpa(mu_idx), 
				ppa,
				((NVM_Transaction_Flash_WR*)transaction)->get_sector_bitmap(mu_idx, sectors_per_mu) | domain->Get_page_status(ideal_mapping_table, transaction->Stream_id, transaction->get_lpa(mu_idx)));
		}
	...
	void Address_Mapping_Unit_Page_Level::Allocate_new_page_for_gc(NVM_Transaction_Flash_WR* transaction, bool is_translation_page)
	{
		if (is_translation_page) {
			MPPN_type mppn = domains[transaction->Stream_id]->GlobalTranslationDirectory[transaction->get_lpa()].MPPN;
			if (mppn == NO_MPPN) {
				PRINT_ERROR("Unexpected situation occured for gc write in Allocate_new_page_for_gc function!")
			}

			allocate_page_in_plane_for_translation_write(transaction, (MVPN_type)transaction->get_lpa(), true);
			transaction->Physical_address_determined = true;
		} else {
			if (!domains[transaction->Stream_id]->Mapping_entry_accessible(ideal_mapping_table, transaction->Stream_id, transaction->get_lpa())) {
				...
				domains[transaction->Stream_id]->CMT->Reserve_slot_for_lpn(transaction->Stream_id, transaction->get_lpa());
				domains[transaction->Stream_id]->CMT->Insert_new_mapping_info(transaction->Stream_id, transaction->get_lpa(), Convert_address_to_ppa(transaction->Address), transaction->write_sectors_bitmap);
			}

			//transaction = merged GC transaction
			...
			if (domains[stream_id]->Mapping_entry_accessible(ideal_mapping_table, stream_id, transaction->get_lpa())) {
				Stats::CMT_hits++;
				Stats::CMT_hits_per_stream[stream_id]++;
				Stats::total_writeTR_CMT_queries++;
				Stats::total_writeTR_CMT_queries_per_stream[stream_id]++;
				Stats::writeTR_CMT_hits++;
				Stats::writeTR_CMT_hits_per_stream[stream_id]++;
				domains[stream_id]->Update_mapping_info(ideal_mapping_table, stream_id, transaction->get_lpa(), transaction->get_ppa(), transaction->write_sectors_bitmap);
				...
				domains[stream_id]->CMT->Reserve_slot_for_lpn(stream_id, transaction->get_lpa());
				domains[stream_id]->CMT->Insert_new_mapping_info(stream_id, transaction->get_lpa(), transaction->get_ppa(), transaction->write_sectors_bitmap);
			}
		}
	}
	...
	void Address_Mapping_Unit_Page_Level::allocate_page_in_plane_for_translation_write(NVM_Transaction_Flash* transaction, MVPN_type mvpn, bool is_for_gc)
	{
		...
		PPA_type ppa = Convert_address_to_ppa(transaction->Address);
		transaction->get_ppa(ppa);
		domain->GlobalTranslationDirectory[mvpn].MPPN = (MPPN_type)transaction->get_ppa();
	...
	PPA_type Address_Mapping_Unit_Page_Level::online_create_entry_for_reads(LPA_type lpa, const stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& read_address, uint64_t read_sectors_bitmap)
	{
		...
		allocate_new_plane_address(stream_id, read_address);
	...
	//FGM - Methods to convert the physical address
	inline NVM::FlashMemory::Physical_Page_Address Address_Mapping_Unit_Page_Level::Convert_ppa_to_address(
			const PPA_type ppa)
	{
		NVM::FlashMemory::Physical_Page_Address address;

		unsigned int mu_no_per_channel = page_no_per_channel * mu_per_page;
		unsigned int mu_no_per_chip = page_no_per_chip * mu_per_page;
		unsigned int mu_no_per_die = page_no_per_die * mu_per_page;
		unsigned int mu_no_per_plane = page_no_per_plane * mu_per_page;
		unsigned int mu_no_per_block = pages_no_per_block * mu_per_page;

		address.ChannelID = (flash_channel_ID_type) (
				ppa / mu_no_per_channel);

		address.ChipID = (flash_chip_ID_type) (
				(ppa % mu_no_per_channel)
				 / mu_no_per_chip);

		address.DieID = (flash_die_ID_type) (
			((ppa % mu_no_per_channel) % mu_no_per_chip) 
			/ mu_no_per_die);

		address.PlaneID = (flash_plane_ID_type) (
			(((ppa % mu_no_per_channel) % mu_no_per_chip) % mu_no_per_die) 
			/ mu_no_per_plane);

		address.BlockID = (flash_block_ID_type) (
			((((ppa % mu_no_per_channel) % mu_no_per_chip) % mu_no_per_die) % mu_no_per_plane) 
			/ mu_no_per_block);

		address.PageID = (flash_page_ID_type) (
			(((((ppa % mu_no_per_channel) % mu_no_per_chip) % mu_no_per_die) % mu_no_per_plane) % mu_no_per_block) 
			/ mu_per_page);

		address.MUID = (flash_mu_ID_type) (
			((((((ppa % mu_no_per_channel) % mu_no_per_chip) % mu_no_per_die) % mu_no_per_plane) % mu_no_per_block) % mu_per_page) );
			...
			NVM::FlashMemory::Physical_Page_Address& address)
	{
		...
		address.DieID = (flash_die_ID_type) (
			((ppn % mu_no_per_channel) % mu_no_per_chip) 
			/ mu_no_per_die);

		address.PlaneID = (flash_plane_ID_type) (
			(((ppn % mu_no_per_channel) % mu_no_per_chip) % mu_no_per_die) 
			/ mu_no_per_plane);

		address.BlockID = (flash_block_ID_type) (
			((((ppn % mu_no_per_channel) % mu_no_per_chip) % mu_no_per_die) % mu_no_per_plane) 
			/ mu_no_per_block);

		address.PageID = (flash_page_ID_type) (
			(((((ppn % mu_no_per_channel) % mu_no_per_chip) % mu_no_per_die) % mu_no_per_plane) % mu_no_per_block) 
			/ mu_per_page);

		address.MUID = (flash_mu_ID_type) (
			((((((ppn % mu_no_per_channel) % mu_no_per_chip) % mu_no_per_die) % mu_no_per_plane) % mu_no_per_block) % mu_per_page) );
			...
			const NVM::FlashMemory::Physical_Page_Address& pageAddress)
	{
		...
		return (PPA_type) ( 
				...
				+ mu_per_page
				* (pageAddress.PageID 
					+ pageAddress.BlockID * pages_no_per_block 
					+ pageAddress.PlaneID * page_no_per_plane 
					+ pageAddress.DieID * page_no_per_die
					+ pageAddress.ChipID * page_no_per_chip 
					+ pageAddress.ChannelID * page_no_per_channel));
	}
					...
					if (_my_instance->domains[transaction->Stream_id]->CMT->Is_slot_reserved_for_lpn_and_waiting(transaction->Stream_id, lpa)) {
						...
						while (it2 != _my_instance->domains[transaction->Stream_id]->Waiting_unmapped_program_transactions.end() &&
							...
							if (_my_instance->is_lpa_locked_for_gc(transaction->Stream_id, lpa)) {
								...
								if (_my_instance->translate_lpa_to_ppa(transaction->Stream_id, it2->second)) {
									_my_instance->ftl->TSU->Submit_transaction(it2->second);
									if (((NVM_Transaction_Flash_WR*)it2->second)->get_related_read() != NULL) {
										_my_instance->ftl->TSU->Submit_transaction(((NVM_Transaction_Flash_WR*)it2->second)->get_related_read());
									}
								} else {
									_my_instance->mange_unsuccessful_translation(it2->second);
								}
	...
	//FGM - Setting a barrier for accessing a physical block during GC operations
		...
		{		
			if (block_manager->Is_page_valid(block, pageID)) 
			{
				...
				if (block->Holds_mapping_data) {
					MVPN_type mpvn = (MVPN_type)flash_controller->Get_metadata(addr.ChannelID, addr.ChipID, addr.DieID, addr.PlaneID, addr.BlockID, addr.PageID, addr.MUID);
				...
				{
					//FGM - Iterate through the LPAs of the mapping units of a page before locking them 
					for (unsigned int mu_idx=0; mu_idx<mu_per_page; mu_idx++)
					{
						if (block_manager->Is_mu_valid(block, pageID, mu_idx))
						{
							...
							addr.MUID = mu_idx;							
							LPA_type lpa = flash_controller->Get_metadata(addr.ChannelID, addr.ChipID, addr.DieID, addr.PlaneID, addr.BlockID, addr.PageID, addr.MUID);
	...
	{
		std::pair<LPA_type, NVM_Transaction_Flash*> entry(transaction->get_lpa(), transaction);
		if (transaction->Type == Transaction_Type::READ) {
	...
	void Address_Mapping_Unit_Page_Level::Start_servicing_writes_for_overfull_plane(const NVM::FlashMemory::Physical_Page_Address plane_address)
	{
		...
		while (program != waiting_write_list.end()) {
			if (translate_lpa_to_ppa((*program)->Stream_id, *program)) {
				...
				if ((*program)->get_related_read() != NULL) {
					ftl->TSU->Submit_transaction((*program)->get_related_read() );
				}
	...
	//FGM - The plane allocation algorithm in a Round-Robin fashion. 
	void Address_Mapping_Unit_Page_Level::allocate_new_plane_address (stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& targetAddress) 
	{
		AddressMappingDomain* domain = domains[stream_id];

		unsigned int* channel_id = &domain->mru_channel_id;
		unsigned int* chip_id = &domain->mru_chip_id;
		unsigned int* die_id = &domain->mru_die_id;
		unsigned int* plane_id = &domain->mru_plane_id; 


		unsigned int channel_no = domain->Channel_no;
		unsigned int chip_no = domain->Chip_no;
		unsigned int die_no = domain->Die_no;
		unsigned int plane_no = domain->Plane_no;
	
		switch (domain->PlaneAllocationScheme)
		{
		//Static: Channel first
		case Flash_Plane_Allocation_Scheme_Type::CWDP:
			calculate_resource_indices (
					channel_no, chip_no, die_no, plane_no,
					channel_id, chip_id, die_id, plane_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::CWPD:
			calculate_resource_indices (
					channel_no, chip_no, plane_no, die_no,
					channel_id, chip_id, plane_id, die_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::CDWP:
			calculate_resource_indices (
					channel_no, die_no, chip_no, plane_no,
					channel_id, die_id, chip_id, plane_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::CDPW:
			calculate_resource_indices (
					channel_no, die_no, plane_no, chip_no,
					channel_id, die_id, plane_id, chip_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::CPWD:
			calculate_resource_indices (
					channel_no, plane_no, chip_no, die_no,
					channel_id, plane_id, chip_id, die_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::CPDW:
			calculate_resource_indices (
					channel_no, plane_no, die_no, chip_no,
					channel_id, plane_id, die_id, chip_id);
			break;
			//Static: Way first
		case Flash_Plane_Allocation_Scheme_Type::WCDP:
			calculate_resource_indices(
					chip_no, channel_no, die_no, plane_no,
					chip_id, channel_id, die_id, plane_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::WCPD:
			calculate_resource_indices(
					chip_no, channel_no, plane_no, die_no,
					chip_id, channel_id, plane_id, die_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::WDCP:
			calculate_resource_indices(
					chip_no, die_no, channel_no, plane_no,
					chip_id, die_id, channel_id, plane_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::WDPC:
			calculate_resource_indices(
					chip_no, die_no, plane_no, channel_no,
					chip_id, die_id, plane_id, channel_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::WPCD:
			calculate_resource_indices(
					chip_no, plane_no, channel_no, die_no,
					chip_id, plane_id, channel_id, die_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::WPDC:
			calculate_resource_indices(
					chip_no, plane_no, die_no, channel_no,
					chip_id, plane_id, die_id, channel_id);			
			break;
			//Static: Die first
		case Flash_Plane_Allocation_Scheme_Type::DCWP:
			calculate_resource_indices(
					die_no, channel_no, chip_no, plane_no,
					die_id, channel_id, chip_id, plane_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::DCPW:
			calculate_resource_indices(
					die_no, channel_no, plane_no, chip_no,
					die_id, channel_id, plane_id, chip_id);
			
			break;
		case Flash_Plane_Allocation_Scheme_Type::DWCP:
			calculate_resource_indices(
					die_no, chip_no, channel_no, plane_no,
					die_id, chip_id, channel_id, plane_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::DWPC:
			calculate_resource_indices(
					die_no, chip_no, plane_no, channel_no,
					die_id, chip_id, plane_id, channel_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::DPCW:
			calculate_resource_indices(
					die_no, plane_no, channel_no, chip_no,
					die_id, plane_id, channel_id, chip_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::DPWC:
			calculate_resource_indices(
					die_no, plane_no, chip_no, channel_no,
					die_id, plane_id, chip_id, channel_id);
			break;
			//Static: Plane first
		case Flash_Plane_Allocation_Scheme_Type::PCWD:
			calculate_resource_indices(
					plane_no, channel_no, chip_no, die_no,
					plane_id, channel_id, chip_id, die_id); 
			break;
		case Flash_Plane_Allocation_Scheme_Type::PCDW:
			calculate_resource_indices(
					plane_no, channel_no, die_no, chip_no,
					plane_id, channel_id, die_id, chip_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::PWCD:
			calculate_resource_indices(
					plane_no, chip_no, channel_no, die_no,
					plane_id, chip_id, channel_id, die_id);
			
			break;
		case Flash_Plane_Allocation_Scheme_Type::PWDC:
			calculate_resource_indices(
					plane_no, chip_no, die_no, channel_no,
					plane_id, chip_id, die_id, channel_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::PDCW:
			calculate_resource_indices(
					plane_no, die_no, channel_no, chip_no,
					plane_id, die_id, channel_id, chip_id);
			break;
		case Flash_Plane_Allocation_Scheme_Type::PDWC:
			calculate_resource_indices(
					plane_no, die_no, chip_no, channel_no,
					plane_id, die_id, chip_id, channel_id );
			break;
		default:
		...
		targetAddress.ChannelID = domain->Channel_ids[*channel_id];
		targetAddress.ChipID = domain->Chip_ids[*chip_id];
		targetAddress.DieID = domain->Die_ids[*die_id];
		targetAddress.PlaneID = domain->Plane_ids[*plane_id]; 
			
	}
	
	void Address_Mapping_Unit_Page_Level::calculate_resource_indices (
			unsigned int first_no,
			unsigned int second_no,
			unsigned int third_no,
			unsigned int fourth_no,
			unsigned int* first_idx,
			unsigned int* second_idx,
			unsigned int* third_idx,
			unsigned int* fourth_idx)
	{
		if (++(*first_idx) == first_no)
		{
			*first_idx = 0;

			if (++(*second_idx) == second_no)
			{
				*second_idx = 0;

				if (++(*third_idx) == third_no)
				{
					*third_idx = 0;

					if (++(*fourth_idx) == fourth_no)
					{
						*fourth_idx = 0;
					}
				}
						...
						for (int valid_pages_in_block = pages_no_per_block; valid_pages_in_block >= 0; valid_pages_in_block--) {
							...
							for (unsigned int block_cntr = 0; block_cntr < block_no_with_x_valid_page; block_cntr++) {
								...
								for (int page_cntr = 0; page_cntr < valid_pages_in_block; page_cntr++) {
									NVM::FlashMemory::Physical_Page_Address addr(plane_address.ChannelID, plane_address.ChipID, plane_address.DieID, plane_address.PlaneID, 0, 0, 0);
